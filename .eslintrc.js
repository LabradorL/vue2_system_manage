...
module.exports = {
  ...
  // plugin:vue/recommended 除了插件默认的，还有包含其他的规则
  ...
  rules: {
    ...
    'no-empty-pattern': 2, // 不能在解构对象和数组中的任何的空模式
    ...
    'no-new-symbol': 2, // 阻止使用 new 操作符调用 Symbol
    ...
    'no-self-assign': 2, // 消除自身赋值的情况
    'no-unmodified-loop-condition': 2, // 发现循环条件的引用，检查这些引用的变量在循环过程中是否发生改变。
    ...
    'no-unsafe-finally': 2, // 禁止在 finally 语句块中出现 return、throw、break 和 continue 语句。它允许间接使用，比如在 function 或 class 的定义中。
    'no-unused-vars': [
      2,
      {
        vars: 'all',
    ...
    'no-useless-constructor': 2, // 可以被安全移除但又不改变类的行为的构造函数
    'no-useless-computed-key': 2, // 禁止使用不必要的计算属性。
    ...
    'block-spacing': [2, 'always'], // always需要一个或多个空格 never 禁止使用空格
    ...
    curly: [2, 'multi-line'], // 非必须使用 if(){} 中的{}
    ...
    'handle-callback-err': [2, '^(err|error)$'], // 期望在 Node.js 中使用回调模式时能够处理错误
    ...
    'template-curly-spacing': [2, 'never'], // 维护模板字面量中空格的一致性。
    strict: 2, // 使用严格模式
    ...
    'yield-star-spacing': [2, 'both'], // 强制在 yield* 表达式中 * 左右使用空格。
