 ...
 * this.option = getPie3D(this.optionData, 0, 540, 28, 26, 1)
 */
  ...
  const option = {
    legend: {
      ...
      formatter: function(param) {
        ...
        // item.value 占比 item.name 名称 item.num 数量
    ...
    tooltip: {
      ...
      formatter: params => {
        if (
          ...
          const tempData = option.series[params.seriesIndex].pieData
          const bfb = tempData ? ((tempData.endRatio - tempData.startRatio) * 100).toFixed(2) : 0
          return (
            `${params.seriesName}<br/>`+
            `${bfb}%</br>`
            // + `${tempData ? tempData.value : 0}`
          )
          // return (
          //   `${params.seriesName}<br/>` +
...
const getParametricEquation = (startRatio, endRatio, isSelected, isHovered, k, h) => {
  ...
  const hoverRate = isHovered ? 1.8 : 1.7
  // 返回曲面参数方程
  return {
    u: {
      min: -Math.PI,
    ...
    z: function(u, v) {
      ...
      return Math.sin(v) > 0 ? Number(h) * 0.1 : -1
    }
  }
}
